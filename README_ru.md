...
- `private _modalContainer: HTMLElement` - Контейнер для структуры модального окна

**Методы**:

- `render(data: IModalContent): void` - Обновляет содержимое и заголовок модального окна, обрабатывает отображение null цен
- `open(): void` - Открывает модальное окно и генерирует событие 'modal:open'
- `close(): void` - Закрывает окно, очищает содержимое и генерирует событие 'modal:close'
...
- Закрывается при клике вне области содержимого модального окна

**Поведение при инициализации**:

- Динамически создает отсутствующие элементы структуры модального окна, если они не найдены в контейнере
- Поддерживает правильную иерархию элементов: container > modal\_\_container > (title, content, close button)
