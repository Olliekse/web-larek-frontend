...
8. [Система событий](#система-событий)
   ...
   - [🏷️ События продуктов](#️-события-продуктов)
...
Магазин мерча для разработчиков, где пользователи могут тратить "синапсы" (виртуальную валюту) на товары для разработчиков. Построен с использованием TypeScript и архитектуры MVP, включает адаптивную галерею, функциональность корзины и валидацию форм.
...
2. **Слой View**: Обрабатывает отображение UI и взаимодействие с пользователем
   ...
   - Захватывает пользовательский ввод
...
- `state:modal:changed` - Изменение состояния модального окна

### 🏷️ События продуктов

- `card:select` - Выбор карточки продукта
- `card:addCart` - Запрос на добавление продукта в корзину
- `state:products:changed` - Обновление списка продуктов

### 📝 События форм

- `contacts:changeInput` - Изменение поля в форме контактов
- `contacts:submit` - Отправка формы контактов
- `formErrors:change` - Обновление ошибок валидации формы
...
API слой реализует надежную обработку ошибок и типобезопасность:

1. **Категории ошибок**:

   - Сетевые ошибки (проблемы с подключением)
   - Серверные ошибки (ответы 5xx)
   - Клиентские ошибки (ответы 4xx)
	...
	if (error instanceof ApiError) {
		switch (error.type) {
			case 'network':
				...
				break;
			case 'server':
				this.appState.setError('Ошибка сервера. Попробуйте позже.');
				break;
			default:
...
3. **Обработка ответов**:
   ...
   - Трансформация данных для слоя представления
   - Единообразное форматирование сообщений об ошибках
...
1. **Манипуляции с DOM**
   ...
   - **Централизация**:
     ...
     - Все запросы к DOM централизованы в классе `App`
     - Элементы передаются через конструкторы компонентов
     - Прямое использование `querySelector` запрещено в компонентах

   - **Использование DOMService**:
     ```typescript
     class SomeView {
     	constructor(
     		protected container: HTMLElement,
     		...
     		// Использование domService вместо прямых манипуляций с DOM
...
2. **Разработка компонентов**
   ...
   - Передавать DOM элементы через конструкторы
   - Использовать интерфейсы TypeScript для лучшей типизации
   - Реализовывать событийно-ориентированную коммуникацию
   - Использовать protected/private поля для лучшей инкапсуляции

3. **Управление состоянием**

   - Использовать AppState для глобального состояния
   - Реализовывать изменения состояния через события
   - Валидировать обновления состояния
   - Хранить вычисляемые свойства в геттерах
...
4. **Тестирование**
   ...
   - Мокировать DOM элементы и сервисы
