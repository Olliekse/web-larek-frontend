...
1. **AppState**
   ...
   - **Назначение**: Централизованно управляет глобальным состоянием приложения, включая каталог продуктов, корзину и состояние загрузки. Обеспечивает единый источник правды для всего приложения и уведомляет об изменениях через систему событий. Расположен в директории моделей как единственный источник истины.
   - **Поля**:
...
2. **FormModel**
   ...
   - **Назначение**: Управляет валидацией, форматированием и состоянием данных формы. Обрабатывает все операции с данными, включая форматирование и валидацию телефонных номеров, обеспечивая правильный формат данных перед отправкой.
   - **Поля**:
     - `protected payment: string` - Способ оплаты
     - `protected email: string` - Email пользователя
     - `protected phone: string` - Телефон пользователя (поддерживает международные форматы)
     - `protected address: string` - Адрес доставки
     - `protected formErrors: FormErrors` - Ошибки валидации
   - **Методы**:
     - `setOrderData(field: string, value: string): void` - Обновляет и форматирует данные формы
     - `validateContacts(): boolean` - Проверяет контактную информацию
     - `validateOrder(): boolean` - Проверяет информацию заказа
     - `getOrderLot(): IOrder` - Подготавливает данные заказа для отправки
...
1. **BaseCard**
   ...
   - **Назначение**: Предоставляет базовую функциональность для всех карточных компонентов, обрабатывая общие манипуляции с DOM и управление шаблонами. Родительский класс для конкретных реализаций карточек.
   - **Поля**:
     - `protected elements: CardElements` - Ссылки на DOM элементы
     - `protected template: HTMLTemplateElement` - Шаблон карточки
     - `protected domService: IDOMService` - Сервис манипуляции с DOM
...
2. **CartItemCard**
   ...
   - **Назначение**: Расширяет BaseCard для обеспечения специфической функциональности элементов корзины. Использует шаблонный рендеринг и обрабатывает только UI взаимодействия.
   - **Поля**:
     - `private readonly elements: CartItemElements` - DOM элементы для корзины
   - **Методы**:
     - `render(data: IProduct): HTMLElement` - Отрисовывает элемент корзины
     - `setIndex(value: number): void` - Обновляет отображение индекса элемента
...
6. **ContactsView**
   - **Назначение**: Чистый компонент представления для формы контактов. Отвечает только за отображение UI и захват пользовательского ввода, делегируя всю обработку данных модели.
   - **Поля**:
   ...
   - **Методы**:
     ...
     - `set valid(value: boolean)` - Обновляет состояние валидации формы
